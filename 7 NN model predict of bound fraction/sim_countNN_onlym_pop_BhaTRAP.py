...
parser.add_argument("-T", help="Temperature /K", type=float, default=298.15)
parser.add_argument("-nsites", help="specify number of sites", type=int, default=12)
parser.add_argument("-kdx", help="first dissociation constant in M", type=float, default=7.17913E-04)
parser.add_argument("-kdy", help="second dissociation constant in M", type=float, default=4.48637E-05)
parser.add_argument("-kdz", help="third dissociation constant in M", type=float, default=2.80362E-06)
parser.add_argument("-lattice", help="concentration of lattice w/ nsites in M", type=float, default=6.0367E-06)
parser.add_argument("-maxlig",
	help="maximum titrated ligand concentration in M", type=float,
	default=140e-6)
parser.add_argument("-npoints", help="number of ligand titration points", type=int, default=200)
...
ligand_concs = numpy.arange(ligand_conc_start,ligand_conc_end+ligand_conc_step,ligand_conc_step) # arange is non-inclusive
#ligand_concs = numpy.linspace(ligand_conc_start,ligand_conc_end,npoints)
lattice_concs = [lattice_conc]*ligand_concs.size	# make the list of lattice concentrations match ligand
...
path= ("simulated-" + str(nsites) + "-ms_cnn.txt")
